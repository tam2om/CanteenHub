...
- **`draft`** — visible to admins only. A day created by hand starts here; a day created by an IMPORT does not (see decision 35). Employees cannot see it and cannot select against it.
...
**Menu** — each day yields one `menu_days` row (keyed by date **and `meal_type`**, §7.1), exactly two `menu_options`, and N `menu_components`. Fewer or more than two options is an `error`, not a silent accept. `Option Meal 1` and `Option Meal 2` map to **components, not options**, despite their names. Imports COMMIT AS `published` (decision 35): the confirm step is the review, so a second manual publish per day was asking for the same decision twice. An import never unpublishes, and never resurrects an archived day. A date that already has selections is flagged as a `conflict` with the count.
...
35. **A menu import publishes what it commits.** The original rule created every imported day as a draft so nothing could go live unreviewed. But the review already happened: an import cannot commit without an administrator reading the preview and confirming it, and requiring them to then publish thirty days one at a time on another screen asked for the same decision twice — while a menu nobody can select from is not a menu. A created day is created `published`; an updated day is promoted from `draft`. The two limits that actually protect someone stay: status only ever moves TOWARDS published, so an import can never take a live menu down; and an `archived` day keeps that status, because archiving is a deliberate act whose reason the importer cannot see. An UNCHANGED row writes no dishes, but IS published if it is sitting in draft - otherwise "the workbook is the menu" would be false in the case people hit hardest, re-uploading a file after an earlier import left its days unpublished. A day already published produces no statement at all, so re-importing a settled workbook stays a true no-op.
34. **The XLSX reader scans tag by tag, never with one alternating regular expression.** Excel writes an empty-but-styled cell inside a merged region as a self-closing `<c r="D2" s="13"/>`. The obvious pattern — `/<c(?:\s([^>]*))?>([\s\S]*?)<\/c>|<c\s([^>]*)\/>/` — lets `[^>]*` consume that trailing slash, so the first branch matches and its lazy body runs on to the NEXT `</c>`: the empty cell swallows the populated one, inherits its `<v>` but not its `t="s"`, and emits a **shared-string index as the value** while the real cell disappears. On the real September workbook this produced `Day | Date | Lunch Menu | "4"` with `Option 1` gone, and it could as easily have put `17` into a dish name. No fixture that omits blank cells can reproduce it, which is why the test fixtures now write them the way Excel does.
